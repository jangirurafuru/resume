...
locals {
  ...
  final_bucket_name = var.bucket_name != null ? var.bucket_name : format("resume-%s-%s-an_${terraform.workspace}", data.aws_caller_identity.current.account_id, data.aws_region.current.region)
}
